--1)
  ------1.
      SELECT UNIQUE NAME FROM COUNTRY WHERE NAME LIKE 'C%' OR NAME LIKE 'D%';
  --2.
      SELECT NAME FROM COUNTRY WHERE GDP > .15 AND YEAR = '2011';
  --3.
      SELECT UNIQUE REGION FROM COUNTRY;
  --4.
      SELECT NAME FROM COUNTRY WHERE POPULATION < 1000000 AND YEAR = '2011';
  --5.
      SELECT NAME FROM COUNTRY WHERE REGION = 'Europe' AND AREA > 100000 AND YEAR = '2005' ORDER BY GDP DESC;

--2)
  --1.
      SELECT COUNT (NAME) FROM COUNTRY WHERE GDP > .15 AND YEAR = '2005';
 --This is not correct english. For each year,...
  --2..
      SELECT YEAR , COUNT(NAME) AS NUMBER_COUNTRIES, SUM(POPULATION) AS TOTAL_POPULATION, ( (SUM(POPULATION) ) / (COUNT(NAME) ) ) AS AVE_POPULATION , SUM(GDP) AS TOTAL_GDP FROM COUNTRY WHERE REGION = 'Europe' GROUP BY REGION, YEAR ORDER BY YEAR DESC;
  --3.
      SELECT A.REGION FROM COUNTRY A WHERE A.YEAR = '2005' GROUP BY A.REGION HAVING COUNT(*) > 4 AND SUM(A.POPULATION) > 45000000;
  --4.
      SELECT YEAR , SUM ((POPULATION /AREA )) AS DENSITY FROM COUNTRY WHERE REGION = 'Europe' GROUP BY YEAR  ORDER BY YEAR DESC;
...
--3)
 -- This takes a little while to figure out what it is asking. It would be more clear if find names of regions and the area of these regions with.. bc name is a field in the table
  ...
  --3.
      SELECT A.NAME FROM COUNTRY A WHERE A.YEAR = '2006' AND A.REGION = 'Europe' AND A.POPULATION > (SELECT B.POPULATION FROM COUNTRY B WHERE A.YEAR = B.YEAR AND B.NAME = 'Spain');
  --4.
      SELECT A.NAME FROM COUNTRY A WHERE A.YEAR ='2008' AND A.AREA NOT IN (SELECT B.AREA FROM COUNTRY B , COUNTRY C WHERE B.YEAR = C.YEAR AND B.YEAR = '2008' AND B.AREA > C.AREA);
...
--5)
  ...
  --1.
      CREATE INDEX POPULATION_RANGE ON COUNTRY (POPULATION); -- THIS IS HAVE AN INDEX_TYPE OF NORMAL WHICH IS BY DEFAULT A B+ TREE IN ORACLE.
...
--TABLES SPACES CAN HAVE MORE THAN ONE TABLE BUT A TABLE CAN ONLY PE A PART OF ONE TABLESAPCEA.
  ...
  --5.
      SELECT NUM_ROWS, BLOCKS, AVG_ROW_LEN, LAST_ANALYZED, DEPENDENCIES  FROM USER_TABLES;
  --6.
      SELECT INDEX_NAME, INDEX_TYPE FROM USER_INDEXES;
  --7.
      SELECT NUM_DISTINCT, NUM_NULLS, DENSITY, LOW_VALUE, HIGH_VALUE FROM USER_TAB_COLUMNS WHERE TABLE_NAME = 'COUNTRY';
  --8.
  ...
  --12.
      SELECT A.LEAF_BLOCKS, A.INDEX_NAME, A.INDEX_TYPE, A.CLUSTERING_FACTOR, B.COLUMN_NAME  FROM USER_INDEXES A, USER_IND_COLUMNS B WHERE A.INDEX_NAME = B.INDEX_NAME;
  --13.
...
--3)
  --1.
      SELECT TABLE_NAME, PRIVILEGE FROM ALL_TAB_PRIVS WHERE GRANTOR = 'ORDDATA';
